...
import { img_base, to } from '../consts'
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-8 w-fit mx-auto">
            {medias.map((media: Media) => {
                return (
                    <div title={`Go to detail page of ${media.name}`} onClick={() => navigate(`/${to[media.mediaType]}${media.id}`)} key={media.id} className="col-span-1 w-90 rounded-lg shadow-lg p-4 border bg-white border-gray-200 cursor-pointer">
